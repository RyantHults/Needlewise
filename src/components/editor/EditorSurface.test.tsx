...
describe('EditorSurface', () => {
  ...
  it('surfaces the project name, selected-unit size, rail controls, header history, and palette rail', () => {
    ...
    expect(screen.getByRole('button', { name: 'Stitch' })).toHaveClass('rail-button-active');
    expect(screen.getAllByRole('button', { name: 'Stitch' })).toHaveLength(1);
    fireEvent.click(screen.getByRole('button', { name: 'Stitch' }));
    expect(screen.getByRole('button', { name: 'Stitch' })).toHaveAttribute('aria-expanded', 'true');
    expect(screen.queryByRole('button', { name: 'Paint' })).not.toBeInTheDocument();
  ...
  it('guides users without a thread', () => { const empty = { width: 16, height: 16, colors: new Uint16Array(1024), palette: [{ id: 1, name: 'Ruby', color: '#b44', active: false }], backstitches: { ids: new Uint32Array() } } as never; render(<EditorSurface workspace={ws} document={empty} />); expect(screen.queryByRole('button', { name: 'Paint' })).not.toBeInTheDocument(); expect(screen.getByRole('button', { name: 'Fill' })).toBeDisabled(); fireEvent.click(screen.getByRole('button', { name: 'Stitch' })); expect(screen.getByRole('button', { name: 'Backstitch' })).toBeDisabled(); });
