...
export function TraceImageControls({ workspace, document, controller, activeTool }: Props) {
  ...
  return <div className="trace-controls" role="toolbar" aria-label="Reference image">
    ...
    <label className="trace-visibility bottom-control" title="Show image"><input aria-label="Show image" type="checkbox" disabled={!descriptor} checked={visible} onChange={(event) => { const value = event.target.checked; setVisible(value); controller?.setTraceImageSettings?.({ visible: value }); void commitSettings({ traceVisible: value }, 'trace-image-visibility'); }} /><span aria-hidden="true">◉</span><small>Show</small></label>
