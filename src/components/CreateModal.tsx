...
import { acceptConversionDraft, DEFAULT_CONVERSION_PALETTE_BUDGET, MAX_CONFETTI_DISTANCE, type ConversionDraft } from '../conversion/image-to-pattern';
...
type Props = { catalog: CatalogSnapshot; mode: 'blank' | 'image'; title: string; width: string; height: string; aida: string; backgroundColor: string; busy: boolean; onMode: (mode: 'blank' | 'image') => void; onClose: () => void; onBlank: (event: React.FormEvent<HTMLFormElement>) => void; onConversionCreate: (draft: ReturnType<typeof acceptConversionDraft>, asset?: ProjectAssetInput) => Promise<string>; onCreated?: (projectId: string) => void; onDurableCreateChange?: (locked: boolean) => void; onTitle: (value: string) => void; onWidth: (value: string) => void; onHeight: (value: string) => void; onAida: (value: string) => void; onBackgroundColor: (value: string) => void };
...
export function CreateModal(props: Props) {
  const { catalog, mode, title, width, height, aida, backgroundColor, busy, onMode, onClose, onBlank, onConversionCreate, onCreated, onDurableCreateChange, onTitle, onWidth, onHeight, onAida, onBackgroundColor } = props;
  const [backgroundColorDraft, setBackgroundColorDraft] = useState(backgroundColor);
  useEffect(() => { setBackgroundColorDraft(backgroundColor); }, [backgroundColor]);
  const backgroundColorIsValid = /^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(backgroundColorDraft);
  const validateBackgroundColor = () => {
    if (backgroundColorIsValid) return true;
    setError('Enter a valid 3- or 6-digit HEX background color before creating your pattern.');
    return false;
  };
  const submitBlank = (event: React.FormEvent<HTMLFormElement>) => {
    if (!validateBackgroundColor()) { event.preventDefault(); return; }
    onBlank(event);
  };
  ...
  const [budget, setBudget] = useState(String(initialBudget)); const [budgetDraft, setBudgetDraft] = useState(String(initialBudget)); const [keepReference, setKeepReference] = useState(true); const [lockAspect, setLockAspect] = useState(true); const [backgroundSourceId, setBackgroundSourceId] = useState<string | null>(null); const [autoCrop, setAutoCrop] = useState(false); const [confettiDistance, setConfettiDistance] = useState<number | null>(null); const [dragging, setDragging] = useState(false); const [naturalSize, setNaturalSize] = useState<{ width: number; height: number } | null>(null);
  // The budget the user asked for, counting a chosen background as one of its
  // colors. Automatic clamps to the matched color count leave this untouched so
  // a later, larger conversion can grow back to it.
  const requestedBudget = useRef(initialBudget);
  ...
  useEffect(() => {
    const restored = String(Math.max(1, Math.min(requestedBudget.current - (backgroundSourceId === null ? 0 : 1), catalog.association.colorCount)));
    if (restored === budget) return;
    setBudget(restored); setBudgetDraft(restored);
  }, [width, height, file]);
  useEffect(() => {
    const canvas = previewRef.current, draft = draftRef.current; if (!canvas || !draft) return; const context = canvas.getContext('2d'); if (!context) return; const size = calculatePreviewSize(draft.document.width, draft.document.height); canvas.width = size.width; canvas.height = size.height; const colors = new Map(draft.document.palette.map(e => [e.id, e.color])); const image = context.createImageData(canvas.width, canvas.height); for (let y = 0; y < canvas.height; y++) for (let x = 0; x < canvas.width; x++) { const sx = Math.min(draft.document.width - 1, Math.floor(x * draft.document.width / canvas.width)); const sy = Math.min(draft.document.height - 1, Math.floor(y * draft.document.height / canvas.height)); const value = colors.get(draft.document.colors[(sy * draft.document.width + sx) * 4])?.replace('#', '') ?? backgroundColor.replace('#', ''); const offset = (y * canvas.width + x) * 4; image.data[offset] = parseInt(value.slice(0, 2), 16); image.data[offset + 1] = parseInt(value.slice(2, 4), 16); image.data[offset + 2] = parseInt(value.slice(4, 6), 16); image.data[offset + 3] = 255; } context.putImageData(image, 0, 0); hidePreviewLens(lensRef.current); }, [draftInfo, backgroundColor]);
  ...
  // pointercancel or an outside tap dismisses it.
  const handlePreviewDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    if (event.pointerType !== 'touch') return;
    const canvas = previewRef.current, lens = lensRef.current, frame = frameRef.current;
    if (!canvas || !lens || !frame) return;
    lensVisible.current = true;
  ...
  const handlePreviewLeave = (event: React.PointerEvent<HTMLCanvasElement>) => {
    if (event.pointerType === 'touch') return;
    hidePreviewLens(lensRef.current);
  };
  // pointercancel fires when the browser takes the gesture over for scrolling;
  // that's the only case a touch tap should hide the lens on its own.
  const handlePreviewCancel = () => { hidePreviewLens(lensRef.current); };
  function hidePreviewLens(lens: HTMLCanvasElement | null) {
    if (!lens) return;
    lensVisible.current = false;
    lens.style.opacity = '0';
  }
  // While a touch-shown lens is visible, a tap anywhere outside the preview
  // canvas dismisses it. Mouse/pen already hide via pointerleave above, so
  // this listener is a no-op for them.
  useEffect(() => {
    const handleOutsideDown = (event: PointerEvent) => {
      if (!lensVisible.current) return;
      const canvas = previewRef.current;
      if (canvas && event.target instanceof Node && canvas.contains(event.target)) return;
      hidePreviewLens(lensRef.current);
    };
    globalThis.document.addEventListener('pointerdown', handleOutsideDown);
    return () => { globalThis.document.removeEventListener('pointerdown', handleOutsideDown); };
  }, []);
  const setUserBudget = (value: number) => {
    requestedBudget.current = value + (backgroundSourceId === null ? 0 : 1);
    setBudget(String(value)); setBudgetDraft(String(value));
  };
  // A chosen background is left unstitched, so it spends one color of the budget.
  const chooseBackground = (next: string | null) => {
    cancelConversion(true);
    setBackgroundSourceId(next);
    if ((next === null) === (backgroundSourceId === null)) return;
    const adjusted = String(Math.max(1, Math.min(Number(budget) + (next === null ? 1 : -1), catalog.association.colorCount)));
    setBudget(adjusted); setBudgetDraft(adjusted);
  };
  const commitBudget = (value: string) => {
    ...
    setUserBudget(parsed); cancelConversion(true);
  };
  const stepBudget = (delta: number) => {
    ...
    setUserBudget(next); cancelConversion(true);
  };
  const stepConfettiDistance = (delta: number) => {
    if (confettiDistance === null) return;
    const next = Math.max(1, Math.min(confettiDistance + delta, MAX_CONFETTI_DISTANCE));
    if (next === confettiDistance) return;
    cancelConversion(true); setConfettiDistance(next);
  };
  function validate() { const maxColors = Math.max(1, catalog.association.colorCount); const w = Number(width), h = Number(height), colors = Number(budget); if (!Number.isInteger(w) || !Number.isInteger(h) || w < 1 || h < 1 || w > 1000 || h > 1000 || w * h > 1_000_000) { setError('Use whole-number dimensions from 1 to 1,000, with no more than 1,000,000 cells.'); return; } if (!Number.isInteger(colors) || colors < 1 || colors > maxColors) { setError(`Color budget must be a whole number from 1 to ${maxColors}.`); return; } return { w, h, colors }; }
  ...
  const handleDragOver = (event: React.DragEvent<HTMLLabelElement>) => {
    // preventDefault marks the zone as a valid drop target; without it the
    // browser opens the dropped file instead of firing drop.
    event.preventDefault();
    if (statusRef.current === 'creating') { event.dataTransfer.dropEffect = 'none'; return; }
    event.dataTransfer.dropEffect = 'copy';
    setDragging(true);
  };
  function convert() { if (!file || statusRef.current === 'creating') return; const config = validate(); if (!config) { setStatus('error'); return; } const identity: ConversionIdentity = { file, mode, width, height, budget, backgroundSourceId, autoCrop, confettiDistance, catalogId: catalog.association.catalogId, brandLabel: catalog.association.brandLabel, colorCount: catalog.association.colorCount }; const id = ++generation.current; abort.current?.abort(); const signal = (abort.current = new AbortController()).signal; setError(''); setStatus('converting'); void convertImageToPattern(file, { projectId: 'new-pattern', baseRevision: 0, targetWidth: config.w, targetHeight: config.h, paletteBudget: config.colors, catalog, backgroundSourceId: backgroundSourceId ?? undefined, autoCrop: autoCrop || undefined, confettiDistance: confettiDistance ?? undefined, sourceImage: { mimeType: file.type } }, {}, { signal }).then(result => { const current: ConversionIdentity = { file, mode, width, height, budget, backgroundSourceId, autoCrop, confettiDistance, catalogId: catalog.association.catalogId, brandLabel: catalog.association.brandLabel, colorCount: catalog.association.colorCount }; if (mounted.current && id === generation.current && sameIdentity(identity, current)) { draftRef.current = result.draft; acceptedIdentity.current = identity; setDraftInfo(result.draft); setStatus('ready'); const matchedMax = Math.max(1, Math.min(catalog.association.colorCount, result.draft.stats.matchedColorCount ?? catalog.association.colorCount)); if (Number(budget) > matchedMax) { setBudget(String(matchedMax)); setBudgetDraft(String(matchedMax)); } } }).catch(reason => { if (mounted.current && id === generation.current && !(reason instanceof ConversionCancelledError)) { setStatus('error'); setError(reason instanceof Error ? reason.message : 'The image could not be converted.'); } }); }
  ...
  }, [mode, file, width, height, budget, backgroundSourceId, autoCrop, confettiDistance, decodeStatus, catalog.association.catalogId, catalog.association.brandLabel, catalog.association.colorCount]);
  async function create() { if (!validateBackgroundColor()) return; const draft = draftRef.current; const identity = file ? { file, mode, width, height, budget, backgroundSourceId, autoCrop, confettiDistance, catalogId: catalog.association.catalogId, brandLabel: catalog.association.brandLabel, colorCount: catalog.association.colorCount } : null; if (!draft || status !== 'ready' || !sameIdentity(acceptedIdentity.current, identity)) return; const id = generation.current; setStatus('creating'); durableLock.current = true; onDurableCreateChange?.(true); try { const projectId = await onConversionCreate(acceptConversionDraft(draft), keepReference && file ? { id: `source-${Date.now().toString(36)}`, name: file.name, mimeType: file.type, data: file } : undefined); durableLock.current = false; onDurableCreateChange?.(false); if (mounted.current && id === generation.current) { onCreated?.(projectId); onClose(); } } catch (reason) { durableLock.current = false; onDurableCreateChange?.(false); if (mounted.current && id === generation.current) { setStatus('ready'); setError(reason instanceof Error ? reason.message : 'The pattern could not be saved locally.'); } } }
  ...
  const currentIdentity: ConversionIdentity | null = file ? { file, mode, width, height, budget, backgroundSourceId, autoCrop, confettiDistance, catalogId: catalog.association.catalogId, brandLabel: catalog.association.brandLabel, colorCount: catalog.association.colorCount } : null;
  ...
  return <div className="modal-backdrop" role="presentation" onClick={e=>{if(e.target===e.currentTarget)close()}}><div ref={dialogRef} className="create-modal conversion-modal" role="dialog" aria-modal="true" aria-labelledby="create-dialog-title" tabIndex={-1}>
    <button className="modal-close" type="button" aria-label="Close create pattern dialog" disabled={status === 'creating'} onClick={close}>×</button><p className="section-label">New project</p><h2 id="create-dialog-title">Start a pattern</h2>
     ...
     <section className="custom-color-section" aria-labelledby="creation-background-color-title"><h3 id="creation-background-color-title">Background color</h3><div className="custom-color-fields"><label htmlFor="creation-background-color-picker">Background color<input className="custom-color-picker" id="creation-background-color-picker" type="color" value={backgroundColor.toLowerCase()} onChange={e => { const value = e.target.value.toUpperCase(); setBackgroundColorDraft(value); onBackgroundColor(value); setError(''); }} /></label><label htmlFor="creation-background-color-hex">HEX Code<input id="creation-background-color-hex" type="text" value={backgroundColorDraft} maxLength={7} onChange={e => { const raw = e.target.value; setBackgroundColorDraft(raw); if (/^#[0-9a-fA-F]{3}$/.test(raw)) { const canonical = `#${[...raw.slice(1)].map(char => char + char).join('').toUpperCase()}`; setBackgroundColorDraft(canonical); onBackgroundColor(canonical); setError(''); } else if (/^#[0-9a-fA-F]{6}$/.test(raw)) { const canonical = raw.toUpperCase(); setBackgroundColorDraft(canonical); onBackgroundColor(canonical); setError(''); } }} placeholder="#F3EEE5" autoComplete="off" /></label></div></section>{mode === 'blank' && error && <p className="modal-error" role="alert">{error}</p>}
     {mode === 'blank' ? <form onSubmit={submitBlank}><label htmlFor="new-project-title">Title<input id="new-project-title" value={title} onChange={e => onTitle(e.target.value)} /></label><div className="dimension-fields"><label htmlFor="new-project-width">Width<input id="new-project-width" value={width} onChange={e => onWidth(e.target.value)} /></label><label htmlFor="new-project-height">Height<input id="new-project-height" value={height} onChange={e => onHeight(e.target.value)} /></label></div><label htmlFor="new-project-aida">Aida count<select id="new-project-aida" value={aida} onChange={e => onAida(e.target.value)}>{[11, 14, 16, 18, 22].map(x => <option key={x}>{x}</option>)}</select></label>{finishedSizePreview !== null && <p className="control-note">Expected finished size: {finishedSizePreview}</p>}<p className="control-note">Whole-number dimensions. Maximum 1,000,000 cells total.</p><button className="button button-primary" type="submit" disabled={busy}>Create blank pattern</button></form> : <section aria-label="Convert image" aria-busy={status === 'converting' || status === 'updating'}><label htmlFor="new-project-title">Title<input id="new-project-title" value={title} onChange={e => onTitle(e.target.value)} /></label><label className={`file-drop${file ? ' file-drop-selected' : ''}${dragging ? ' file-drop-active' : ''}`} htmlFor="conversion-image" onDragEnter={handleDragOver} onDragOver={handleDragOver} onDragLeave={e => { if (!(e.relatedTarget instanceof Node && e.currentTarget.contains(e.relatedTarget))) setDragging(false); }} onDrop={e => { e.preventDefault(); setDragging(false); const dropped = e.dataTransfer.files.item(0); if (dropped) chooseFile(dropped); }}><svg className="file-drop-icon" aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 15V4" /><path d="m7 9 5-5 5 5" /><path d="M4 15v3a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-3" /></svg>{file ? <span className="file-drop-text"><strong>{file.name}</strong><span className="file-drop-hint">Drop another image to replace it</span></span> : <span className="file-drop-text"><span className="file-drop-title">Drop an image here</span><span className="file-drop-hint">PNG, JPEG, or WebP</span></span>}<span className="file-drop-button">{file ? 'Replace image' : 'Browse files'}</span><input id="conversion-image" className="visually-hidden" type="file" accept="image/png,image/jpeg,image/webp" aria-label="Choose a PNG, JPEG, or WebP image" disabled={status === 'creating'} onChange={e => chooseFile(e.target.files?.[0])} /></label>{file && decodeStatus === 'ready' && <div className="conversion-controls"><fieldset className="dimension-fields aspect-dimensions"><legend>Total stitch count</legend><label htmlFor="conversion-width">Width<input id="conversion-width" value={width} onChange={e => { cancelConversion(true); invalidatePendingDecode(); deriveLockedDimension(e.target.value, 'width'); onWidth(e.target.value); }} /></label><div className="aspect-lock-control"><span className="aspect-lock-label">aspect ratio</span><button type="button" className="aspect-lock-toggle" aria-pressed={lockAspect} aria-label="Lock aspect ratio" title={lockAspect ? 'Locked: output width and height keep the source image ratio' : 'Unlocked: output width and height are independent'} disabled={!naturalSize} onClick={() => setLockAspect(value => !value)}>{lockAspect ? '🔒' : '🔓'}</button></div><label htmlFor="conversion-height">Height<input id="conversion-height" value={height} onChange={e => { cancelConversion(true); invalidatePendingDecode(); deriveLockedDimension(e.target.value, 'height'); onHeight(e.target.value); }} /></label></fieldset><label htmlFor="conversion-aida">Aida count<select id="conversion-aida" value={aida} onChange={e => onAida(e.target.value)}>{[11, 14, 16, 18, 22].map(x => <option key={x}>{x}</option>)}</select></label><label className="check-row"><input type="checkbox" checked={autoCrop} onChange={e => { cancelConversion(true); setAutoCrop(e.target.checked); }} />Auto-crop to content edges</label><label className="check-row"><input type="checkbox" checked={confettiDistance !== null} onChange={e => { cancelConversion(true); setConfettiDistance(e.target.checked ? 1 : null); }} />Reduce confetti stitches</label>{confettiDistance !== null && <div className="budget-control confetti-control"><label htmlFor="conversion-confetti-distance">Confetti distance <span className="budget-stepper"><button type="button" className="budget-step" aria-label="Decrease confetti distance" disabled={confettiDistance <= 1} onClick={() => stepConfettiDistance(-1)}>−</button><span className="budget-count">{confettiDistance}</span> <span className="budget-suffix">{confettiDistance === 1 ? 'stitch' : 'stitches'}</span><button type="button" className="budget-step" aria-label="Increase confetti distance" disabled={confettiDistance >= MAX_CONFETTI_DISTANCE} onClick={() => stepConfettiDistance(1)}>+</button></span></label><input id="conversion-confetti-distance" type="range" min="1" max={MAX_CONFETTI_DISTANCE} step="1" value={confettiDistance} aria-label={`Confetti distance, ${confettiDistance} ${confettiDistance === 1 ? 'stitch' : 'stitches'}`} onChange={e => { cancelConversion(true); setConfettiDistance(Number(e.target.value)); }} /><p className="control-note">Removes lone stitches with no matching color within {confettiDistance} {confettiDistance === 1 ? 'stitch' : 'stitches'}. Lower values clean up more.</p></div>}<label className="check-row"><input type="checkbox" checked={keepReference} onChange={e => setKeepReference(e.target.checked)} />Keep source image as a reference</label>{status === 'updating' && <span className="sr-only" role="status" aria-live="polite">Updating preview</span>}{draftInfo && <div className="review"><h3>Review conversion</h3><p>{draftInfo?.stats.sourceWidth} × {draftInfo?.stats.sourceHeight} source → {draftInfo?.document.width} × {draftInfo?.document.height} stitches</p>{finishedSizePreview !== null && <p className="control-note">Expected finished size: {finishedSizePreview}</p>}<div ref={frameRef} className="conversion-preview-frame"><canvas ref={previewRef} className="conversion-preview" aria-label="Converted pattern preview" width={draftInfo ? calculatePreviewSize(draftInfo.document.width, draftInfo.document.height).width : undefined} height={draftInfo ? calculatePreviewSize(draftInfo.document.width, draftInfo.document.height).height : undefined} onPointerDown={handlePreviewDown} onPointerEnter={handlePreviewEnter} onPointerMove={handlePreviewMove} onPointerLeave={handlePreviewLeave} onPointerCancel={handlePreviewCancel} /><canvas ref={lensRef} className="preview-lens" width="160" height="160" aria-hidden="true" style={{ opacity: 0, pointerEvents: 'none' }} /></div><div className="budget-control"><label htmlFor="conversion-budget-range">Color budget <span className="budget-stepper"><button type="button" className="budget-step" aria-label="Decrease color budget" disabled={Number(budget) <= 1} onClick={() => stepBudget(-1)}>−</button><input id="conversion-budget-count" className="budget-count" type="text" inputMode="numeric" aria-label="Color budget count" value={budgetDraft} onChange={e => { setBudgetDraft(e.target.value); }} onBlur={() => commitBudget(budgetDraft)} onKeyDown={e => { if (e.key === 'Enter') commitBudget(budgetDraft); }} /> <span className="budget-suffix">colors</span><button type="button" className="budget-step" aria-label="Increase color budget" disabled={Number(budget) >= maxBudgetColors} onClick={() => stepBudget(1)}>+</button></span></label><input id="conversion-budget-range" type="range" min="1" max={maxBudgetColors} step="1" value={budget} aria-label={`Color budget, ${budget} colors`} onChange={e => { cancelConversion(true); setUserBudget(Number(e.target.value)); }} />{previewIsCurrent && palette.length < Number(budget) && <p className="control-note colors-used-note"><strong>{palette.length} of {budget} colors used.</strong> Some colors had no stitches left after color matching or confetti reduction.</p>}</div><h4 className="review-swatches-label">Detected colors</h4><p className="review-swatches-hint">Click a color to set it as the background.</p><div className="review-swatches">{palette.map(entry => { const selected = backgroundSourceId !== null && entry.catalog?.sourceId === backgroundSourceId; return <button key={entry.id} type="button" className={selected ? 'swatch swatch-selected' : 'swatch'} style={{ backgroundColor: entry.color }} aria-label={`Use ${entry.name} as background`} aria-pressed={selected} title={`Use ${entry.name} as background`} onClick={() => chooseBackground(selected ? null : (entry.catalog?.sourceId ?? null))} />; })}{backgroundSourceId !== null && <button type="button" className="review-clear-background" onClick={() => chooseBackground(null)}>Clear background selection</button>}</div><button className="button button-primary" type="button" disabled={!previewIsCurrent || status !== 'ready'} onClick={() => void create()}>Create from image</button></div>}</div>}{error && <p className="modal-error" role="alert">{error}</p>}</section>}
  </div></div>;
}
